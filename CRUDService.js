...
function cadastrarNovoEvento() {
    ...
    for(i=0;i<listaDeUsuarios.length; i++){
        if(listaDeUsuarios[i].id == dadoRecebido)
        posicao = i
    }
    nomeUsuario = listaDeUsuarios[posicao].nome
    var idUsuario = listaDeUsuarios[posicao].id
    ...
    if (usuarioLogado != '' || usuarioLogado != null || usuarioLogado != undefined) {
        ...
        if (dadoRecebido == this.usuarioLogado.id) {
            ShowModal()
        }
    }
}
...
function leitura(){
    url = 'http://192.168.242.62'
    var xhttp = new XMLHttpRequest();
    xhttp.onreadystatechange = function() {
      if (this.readyState == 4 && this.status == 200) {
        dadoRecebido = this.responseText;
        if(dadoRecebido != " 00 00 00 00"){
            cadastrarNovoEvento()
        }
      }
    };
